...
int main() {
    ...
    while (true) {
        ...
        float velL = (left_RPM / 60.0f) * (PI * WHEEL_DIAMETER);
        float velR = (right_RPM / 60.0f) * (PI * WHEEL_DIAMETER);

        float RTV = (velL + velR)/2;
        float RAV = (velR - velL)/WHEEL_DISTANCE;

        LCD.locate(0,0);
        LCD.printf("L:%.2f m/s R:%.2f m/s", velL, velR); //left speed right speed  on screen
        LCD.locate(0, 10);
        LCD.printf("RTV: %.2f m/s", RTV);
        LCD.locate(0, 20);
        LCD.printf("RAV: %.2f m/s", RAV);
    }
}